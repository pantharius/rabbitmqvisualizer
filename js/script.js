...
let _config = {
  ...
  nodesize: 30,
  forcecollide: 35,
  forceChargeStart: -150,
  forceChargeAfter: 0,
  linkSizeStart: 20,
  linkSizeAfter: 80,
  linklengthpadding: 100,
  nodeOpacityNoBinding: 0.2,
  nodeOpacityWithBindings: 1,
  nodeTextInsideBubbleSize: 13,
  nodeTextInsideBubbleColor: "black",
  nodeTextOuterBubbleSize: 20,
  nodeTextOuterBubbleColor: "black",
  ShowConsumers: true,
};
...
let vhosts = WS.call(
  ...
  false,
  false,
  _config.rabbitUsername + ":" + _config.rabbitPassword,
  _config.baseApiUrl
).map((c) => c.name);
...
vhosts.forEach((vhost) => {
  $("#vhostsbar").append(
    $('<a class="nav-link" href="?' + vhost + '">' + vhost + "</a>")
  );
});
...
let rabbitpervhost = new Map(
  vhosts.map((vhost) => [
    vhost,
    {
      ...
      consumers: rabbitmqglobal.consumers.filter((c) => c.queue.vhost == vhost),
...
var Graph = {
  ...
  CreateGraphFromData() {
    let echangealternatesFromPolicies = Graph.currentConfig.policies
      .filter((policy) => policy.definition["alternate-exchange"])
      .map((policy) =>
        Graph.currentConfig.exchanges
          .filter((c) => new RegExp(policy.pattern).test(c.name))
          .map((c) => [
            c,
            Graph.currentConfig.exchanges.find(
              (ex) => ex.name == policy.definition["alternate-exchange"]
            ),
            policy.name,
          ])
      )
    ...
    let echangedeadletterFromPolicies = Graph.currentConfig.policies
      .filter((policy) => policy.definition["dead-letter-exchange"])
      .map((policy) =>
        Graph.currentConfig.exchanges
          .filter((c) => new RegExp(policy.pattern).test(c.name))
          .map((c) => [
            c,
            Graph.currentConfig.exchanges.find(
              (ex) => ex.name == policy.definition["dead-letter-exchange"]
            ),
            policy.definition["dead-letter-routing-key"] &&
            policy.definition["dead-letter-routing-key"].trim().length > 0
              ? policy.definition["dead-letter-routing-key"]
              : policy.name,
          ])
      )
    ...
    let echangedeadletterFromQueues = Graph.currentConfig.queues
      .filter((queue) => queue.arguments["x-dead-letter-exchange"])
      .map((queue) => [
        queue.name,
        queue.arguments["x-dead-letter-exchange"],
        queue.arguments["x-dead-letter-routing-key"] &&
        queue.arguments["x-dead-letter-routing-key"].trim().length > 0
          ? queue.arguments["x-dead-letter-routing-key"]
    ...
    let nodeindex = 1;
    json = {
      nodes: Graph.makeUniqueByKey(
        "name",
        [
          ...[
            ...new Map(
              Graph.currentConfig.bindings
                .map((binding) => [
                  ...Graph.currentConfig.exchanges
                    .filter((e) => e.name == binding.source)
                    .map((e) => ({ ...e, type: "e" })),
                  ...Graph.currentConfig.queues.filter(
                    (q) => q.name == binding.destination
                  ),
                ])
                .flat(1)
                .map((c) => [c.name, c])
            ...
            .filter(
              (c) =>
                ...
                [
                  ...new Map(
                    [
                      ...echangealternatesFromPolicies,
                      ...echangedeadletterFromPolicies,
                    ].map((c) => [c[1].name, c[1]])
                  ).values(),
                ].find((x) => x.name == c.name) == null
            )
            .map((u) =>
              u
                ? {
          ...
          ...[
            ...new Map(
              echangealternatesFromPolicies.map((c) => [c[1].name, c[1]])
            ).values(),
          ].map((c) => ({ id: nodeindex++, name: c.name, color: 3 })),
          ...[
            ...new Map(
              echangedeadletterFromPolicies.map((c) => [c[1].name, c[1]])
            ).values(),
          ].map((c) => ({ id: nodeindex++, name: c.name, color: 4 })),
          ...Graph.currentConfig.exchanges
            .filter(
              (c) =>
                Graph.getBindingsCountExchangeFromPolicies(
                  c,
                  echangealternatesFromPolicies,
                  echangedeadletterFromPolicies
                ) > 0
            )
            .map((c) => ({ id: nodeindex++, name: c.name, color: 1 })),
          ...Graph.currentConfig.queues
            .filter((c) => Graph.getBindingsCountQueueFromConsumers(c) > 0)
            .map((c) => ({ id: nodeindex++, name: c.name, color: 0 })),
          ...(_config.ShowConsumers
            ? Graph.currentConfig.consumers.map((c) => ({
                id: nodeindex++,
                name: Graph.toConsumerName(c),
                color: 2,
              }))
            : []),
          ...Graph.currentConfig.exchanges
            .filter(
              (c) =>
                Graph.getBindingsCountExchange(
                  c,
                  echangealternatesFromPolicies,
                  echangedeadletterFromPolicies
                ) == 0
          ...
          ...Graph.currentConfig.queues
            .filter((c) => Graph.getBindingsCountQueue(c) == 0)
            .map((c) => ({
              id: nodeindex++,
              name: c.name,
              color: 0,
    ...
    json.links = [
      ...Graph.currentConfig.bindings
        .filter((c) => c.source != "")
        .map((c) => ({
          source: json.nodes.find((x) => x.name == c.source).id,
          target: json.nodes.find((x) => x.name == c.destination).id,
          type: c.routing_key,
          color: 0,
        })),
      ...(_config.ShowConsumers
        ? Graph.currentConfig.consumers.map((c) => ({
            source: json.nodes.find((x) => x.name == c.queue.name).id,
            target: json.nodes.find((x) => x.name == Graph.toConsumerName(c))
              .id,
            type: c.consumer_tag,
            color: 1,
          }))
        : []),
      ...echangealternatesFromPolicies.map((c) => ({
        source: json.nodes.find((x) => x.name == c[0].name).id,
        target: json.nodes.find((x) => x.name == c[1].name).id,
        type: c[2],
        color: 2,
      })),
      ...echangedeadletterFromPolicies.map((c) => ({
        source: json.nodes.find((x) => x.name == c[0].name).id,
        target: json.nodes.find((x) => x.name == c[1].name).id,
        type: c[2],
        color: 3,
      })),
      ...echangedeadletterFromQueues.map((c) => ({
        source: json.nodes.find((x) => x.name == c[0]).id,
        target: json.nodes.find((x) => x.name == c[1]).id,
        type: c[2],
        color: 3,
      })),
    ];
        ...
        d3
          ...
          .distance(
            (d) =>
              Graph.getTextWidth(
                d.type,
                "normal " + _config.linklabelsize + "pt arial"
              ) + _config.linklengthpadding
          )
      )
  ...
  nodesAfterForce(nodeId, sieved) {
    ...
    let newLinks = json.links.filter((link) => link.source.id === nodeId);
    if (newLinks.length == 0) {
      newLinks = json.links.filter((link) => link.target.id === nodeId);
    }
    ...
    let newNodes = newLinks
      .map((link) =>
        json.nodes.filter(
          (newNode) =>
            newNode.id === link.target.id || newNode.id === link.source.id
        )
      )
      .flat(1)
      .filter(
        (node) => sieved.nodes.filter((c) => c.id == node.id).length == 0
      );
  ...
  initialize(json, linksContainer, nodesContainer) {
    ...
    node
      ...
      .style("opacity", (d) =>
        d.alone ? _config.nodeOpacityNoBinding : _config.nodeOpacityWithBindings
      )
      .style("fill", function (d) {
        return _config.colorscheme[d.color];
      })
    ...
    Graph.force.nodes(json.nodes).on("tick", Graph.ticked);

    Graph.force.force("link").links(json.links);
  },
  mouseEnter(event, d) {
  ...
  getTextWidth(text, font) {
    ...
    const canvas =
      Graph.getTextWidth.canvas ||
      (Graph.getTextWidth.canvas = document.createElement("canvas"));
    const context = canvas.getContext("2d");
    context.font = font;
    const metrics = context.measureText(text);
    return metrics.width;
  },
  makeUniqueByKey: (key, array) => [
    ...new Map(array.map((c) => [c[key], c])).values(),
  ],
  // Method that convert the consumer name to a string
  toConsumerName: (c) =>
    c.channel_details.peer_host + ":" + c.channel_details.peer_port,
  // Method that return the bindings count for exchanges
  getBindingsCountExchangeFromBindings: (c) =>
    Graph.currentConfig.bindings.filter((binding) => binding.source == c.name)
      .length,
  getBindingsCountExchangeFromPolicies: (
    c,
    echangealternatesFromPolicies,
    echangedeadletterFromPolicies
  ) =>
    [
      ...new Map(
        [...echangealternatesFromPolicies, ...echangedeadletterFromPolicies]
          .filter((x) => x[0].name == c.name)
          .map((x) => [x[1].name, x[1]])
      ).values(),
    ].length,
  getBindingsCountExchange: (c, b, a) =>
    Graph.getBindingsCountExchangeFromBindings(c) +
    Graph.getBindingsCountExchangeFromPolicies(c, b, a),
  // Method that return the bindings count for queues
  getBindingsCountQueueFromBindings: (c) =>
    Graph.currentConfig.bindings.filter(
      (binding) => binding.destination == c.name
    ).length,
  getBindingsCountQueueFromConsumers: (c) =>
    Graph.currentConfig.consumers.filter(
      (consumer) => consumer.queue.name == c.name
    ).length,
  getBindingsCountQueue: (c) =>
    Graph.getBindingsCountQueueFromBindings(c) +
    Graph.getBindingsCountQueueFromConsumers(c),
};
